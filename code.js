...
figma.on('selectionchange', () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    for (const node of figma.currentPage.selection) {
        if (node.type === 'RECTANGLE' && node.fills && Array.isArray(node.fills)) {
            ...
            const imageFill = node.fills.find((fill) => fill.type === 'IMAGE' && fill.imageHash !== null);
            if (imageFill) {
    ...
    if (imageNodeToPreview) {
        const imageFill = imageNodeToPreview.fills.find((f) => f.type === 'IMAGE');
...
figma.ui.onmessage = (msg) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (msg.type === 'optimize-images') {
        ...
        for (const node of selectedNodes) {
            if (node.type === 'RECTANGLE' && node.fills && Array.isArray(node.fills)) {
                const imageFills = node.fills.filter((fill) => fill.type === 'IMAGE' && fill.imageHash !== null);
    ...
    // UI로부터 최적화된 이미지 데이터를 수신하는 메시지 핸들러 (기존 노드 업데이트용)
        ...
        const node = yield figma.getNodeByIdAsync(imageNodeId);
        if (node && node.type === 'RECTANGLE' && node.fills && Array.isArray(node.fills)) {
            try {
                ...
                for (const fill of node.fills) {
                    if (fill.type === 'IMAGE') {
                        newFills.push(Object.assign(Object.assign({}, fill), { imageHash: newImageHash }));
                    }
                    else {
                        newFills.push(fill);
                    }
    ...
    // UI로부터 새로운 레이어로 임포트될 최적화된 이미지 데이터를 수신하는 핸들러
        ...
        try {
            ...
            const originalNode = yield figma.getNodeByIdAsync(imageNodeId);
            if (originalNode) {
    ...
    // 'Export to External Folder' 버튼 클릭 시 UI에서 요청하는 메시지
        ...
        const resizePercentage = msg.resizePercentage !== undefined ? msg.resizePercentage : 100;
        const quality = msg.quality !== undefined ? msg.quality : 80;
        console.log('Figma: UI로부터 메시지 수신: Export to External Folder', { resizePercentage, quality });
        const selectedNodes = figma.currentPage.selection;
        let imageNodeToExport;
        for (const node of selectedNodes) {
            if (node.type === 'RECTANGLE' && node.fills && Array.isArray(node.fills)) {
        ...
        const imageFill = imageNodeToExport.fills.find((f) => f.type === 'IMAGE');
        if (!imageFill || imageFill.imageHash === null) {
